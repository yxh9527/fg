...
const resolveAssetUrl = (value, ossUrl) => {
  if (value === null || value === undefined || value === "") return "";
...
const buildSlotCardImageUrl = (gameId, symbolId, size) => {
...
const resolveDetailPayload = (row) => {
...
const normalizeSettlementRow = (row, ossUrl) => {
  const source = row || {};
...
const isFruitGameId = (gameId) => {
  const id = Number(gameId);
  return id >= 7000 && id < 8000;
};

const isSlotDetail = (info) => {
...
const isFruitDetail = (info, gameId) => {
  if (!info || typeof info !== "object") return false;
